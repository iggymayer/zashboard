<template>
  <div class="flex flex-col gap-3">
    <div class="base-container grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 p-3 text-xs">
      <template
    ...
    <template v-if="can('runtimeSettings')">
      <div class="settings-section-label">{{ $t('daeRuntimeSettings') }}</div>
      <div class="base-container flex flex-col gap-3 p-3">
...
const runtimeRows = computed(() => {
  ...
  const rows = [
    ...
    { label: t('daeUptime'), value: `${runtime.lifecycle.uptime_seconds ?? '-'} s` },
